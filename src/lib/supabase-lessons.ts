...
export interface Lesson {
  ...
  category: 'grammar' | 'vocabulary' | 'phrases';
  order_index: number;
  created_at?: string;
}
...
// Fetch a specific lesson with its test and questions (и вариантами)
  ...
  try {
    ...
    if (test) {
      ...
      if (questionsError) {
        ...
        // Для каждого вопроса подтягиваем варианты
        questions = await Promise.all(
          questionData.map(async (q) => {
            const { data: options, error: optionsError } = await supabase
              ...
              .eq('question_id', q.id);
            if (optionsError) {
              console.error('Ошибка загрузки вариантов для вопроса', q.id, optionsError);
              return { ...q, options: [] };
            }
    ...
    return {
      lesson,
      test: test ? { ...test, questions } : undefined,
...
export const saveLessonTestResult = async (
  ...
  try {
    ...
    if (!isPerfectScore) {
      // Не сохраняем прогресс, если есть ошибки
      return false;
    }
    ...
    if (existingResult) {
      const { error } = await supabase
        ...
        .update({
          is_completed: true,
          last_attempt_at: now,
          created_at: existingResult.created_at || now
        })
