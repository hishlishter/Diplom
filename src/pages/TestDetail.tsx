...
const TestDetail = () => {
  ...
  const saveResultMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // Инвалидируем для всех годов (2020 - следующий год)
      if (user?.id) {
        const currentYear = new Date().getFullYear();
        for (let year = 2020; year <= currentYear + 1; year++) {
          queryClient.invalidateQueries({ queryKey: ['all-test-results', user.id, year] });
        }
        ...
        if (oldProfile) {
          queryClient.setQueryData(['profile', user.id], {
            ...oldProfile,
            tests_completed: (oldProfile.tests_completed || 0) + 1
          });
        }
      }
    },
