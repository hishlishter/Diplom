...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const ensureUserProfile = async (user: User, name?: string): Promise<Profile | null> => {
    ...
    try {
      ...
      const displayUsername = name || user.user_metadata?.username || user.user_metadata?.name || user.email?.split('@')[0] || 'User';
  ...
  const signUp = async (email: string, password: string, name?: string) => {
    ...
    try {
      ...
      if (!data.user.email_confirmed_at && !data.user.confirmed_at) {
        ...
        // Можно реализовать повторную отправку письма, если потребуется
        // navigate('/auth');
        return;
      }
      ...
      // Create profile только если email подтверждён
  ...
  const resendConfirmationEmail = async (email: string) => {
    try {
      ...
      if (error) {
        toast.error('Ошибка при повторной отправке письма', { description: error.message });
      } else {
        toast.success('Письмо отправлено повторно', { description: 'Проверьте почту' });
      }
    } catch (error) {
